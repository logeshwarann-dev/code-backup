...
def get_config_fields(config_client):
    """Retrieve configuration fields from Redis or use default values."""
    config_fields = {
        "tempid": Constants.DEFAULT_TEMPLATEID,
        "at": Constants.DEFAULT_ACCOUNT_TYPE,
        "tif": Constants.DEFAULT_TIME_INFORCE,
        "cc": Constants.DEFAULT_CLIENTCODE,
    }
    try:
        redis_config = config_client.hgetall("config")
        for field in config_fields.keys():
            config_fields[field] = redis_config.get(field, config_fields[field])
    except Exception as e:
        print(f"Error retrieving config fields from Redis: {e}")
    return config_fields


def get_value_columns_mapping():
    """Define the mapping of CSV columns to Redis fields."""
    return {
        "FinInstrmId": "inid",
        "StrkPric": "pr",
        "MinLot": "minl",
