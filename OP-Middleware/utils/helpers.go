...
func UseExistingPodsInStatefulSet(desiredPodCount int) (bool, int) {
	...
	return true, len(inactivePodsMap)

}

func UseNewPodsInStatefulSet(desiredPodCount int) error {
	clientset, err := CreateK8sClient()
	if err != nil {
		return fmt.Errorf("unable to create k8s client %v", err.Error())
	}

	totalSenderPods, err := CreateNewPodsToStatefulSet(clientset, static.K8S_NAMESPACE, static.SENDER_POD_LABEL, desiredPodCount)
	if err != nil {
		return fmt.Errorf("unable to add new pods %v", err.Error())
	}

	if err := CreatePodMap(int(totalSenderPods)); err != nil {
		return fmt.Errorf("unable to create pod map %v", err.Error())
	}
	...
	if err := SetConfigInSenderPods(static.ConfigData, newPodsMap, responses); err != nil {
		return fmt.Errorf("error setting config in sender pods: %v", err.Error())
	}

	if err := SetRecordsInPods(newPodsMap, static.RECORDS, responses); err != nil {
		return fmt.Errorf("error setting records in sender pods: %v", err.Error())
	}
...
func CreatePodMap(targetCount int) error {
	for range 5 {
		...
		isPodActive = GetPodHealthStatus(static.PEAK_POD_LABEL, static.PEAK_POD_COUNT)
		if !isPodActive {
			continue
		}
		allPods, err = GetPodAddr(static.PEAK_POD_LABEL, static.K8S_NAMESPACE)
		if err != nil {
			fmt.Println("Error in getting pod address", err.Error())
			return err
		}
		for _, pod := range allPods {
			static.PeakPodIPMap[pod.Name] = pod.Status.PodIP
		}

		if len(static.TraderPodIPMap) == static.TRADER_POD_COUNT && len(static.SenderPodIPMap) == targetCount && len(static.PeakPodIPMap) == static.PEAK_POD_COUNT {
	...
	fmt.Println("SenderPodIPMap=", static.SenderPodIPMap, "| TraderPodIPMap=", static.TraderPodIPMap, "| PeakPodIPMap=", static.PeakPodIPMap)
	return nil
}
...
func SetConfigInPeakPods(data static.Config, peakPodMap map[string]string, responses []map[string]interface{}) error {
	var wg sync.WaitGroup
	var mu sync.Mutex

	for podName, podIP := range peakPodMap {
		podAddr := "http://" + podIP + ":" + static.ServicePort + "/api/v1/setConfig"
		var traders []static.Trader

		if data.TargetEnv == 1 {
			traders = AssignMembersToPods(traders, static.LabSessionIdSets, podName, static.PEAK_POD_MEMBERID)
		} else if data.TargetEnv == 2 {
			traders = AssignMembersToPods(traders, static.ProdSessionIdSets, podName, static.PEAK_POD_MEMBERID)
		} else {
			traders = AssignMembersToPods(traders, static.SimSessionIdSets, podName, static.PEAK_POD_MEMBERID)
		}
