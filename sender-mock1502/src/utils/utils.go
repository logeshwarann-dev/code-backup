...
func ConnectionGatewayResponse(data []byte) (*static.GW_Response, error) {

	res := &static.GW_Response{}
...
func CreateSessionLogonRequest(memberId, traderId, msg_seq int, password, key, iv string, CTX *aes.CipherContext) ([]byte, error) {
	...
	// 18. ApplicationSystemVersion 30-byte FixedString (0-terminable) original value: "A" | Lab env: "8016" | Prod env: "8026" |  Not working values: 7899 or 9000
	...
	if static.ORDER_PUMPING_TYPE == static.ASYNC_ORDER_PUMPING_OEOMOC_TYPE {
		if static.TARGET_ENV == static.LAB_ENV {
			applicationSystemVersion = static.LAB_DISABLED_ACTIVITY_SYSTEM_VERSION
	...
	// 19. ApplicationSystemVendor 30-byte FixedString (0-terminable) values: "A" or "TWSIML"
...
func SingleLegLeanOrder(memberId, traderId, msg_seq, inst_id, price, qty, order_type int, CTX *aes.CipherContext, ClOrdID uint64, cc string) ([]byte, error) {
	...
	clOrdID := uint64(static.UINT64_MAX)
	if static.UNIQUE_CLIENT_IDENTIFIER {
		clOrdID = ClOrdID
	}
	if err := ParseUnsignedInt(buf, clOrdID, 8); err != nil {
		return nil, fmt.Errorf("error parsing ClOrdID: %w", err)
	}
	...
	timeInForce := uint8(7)
	if static.ORDER_PUMPING_TYPE == static.ASYNC_ORDER_PUMPING_IOC_TYPE {
		timeInForce = uint8(3)
	}
	if err := ParseUnsignedInt(buf, uint64(timeInForce), 1); err != nil {
		return nil, fmt.Errorf("error parsing TimeInForce: %w", err)
...
func ModifyLeanOrder(memberId, traderId, msg_seq, inst_id, price, qty, order_type int, order_id, act_time uint64, CTX *aes.CipherContext, ClOrdID, OrigClOrdID uint64) ([]byte, error) {
	...
	//9. OrigClOrdID: 8-byte unsigned integer
	origClOrdID := uint64(static.UINT64_MAX)
	if static.UNIQUE_CLIENT_IDENTIFIER {
		origClOrdID = OrigClOrdID
	}
	if err := ParseUnsignedInt(buf, origClOrdID, 8); err != nil {
...
func CancelSingleLegOrderRequest(memberId, traderId, msg_seq, inst_id, product_id int, order_id uint64, CTX *aes.CipherContext, ClOrdID, OrigClOrdID uint64) ([]byte, error) {
	buf := new(bytes.Buffer)
...
func ImmediateExecutionResponse(data []byte) (uint64, uint64, string, error) {
...
func OrderBookExecution(data []byte) (uint64, uint64, string, error) {
...
func DeleteAllOrderComplexRequest(instrument_id, product_id, memberId, traderId, msg_seq int, CTX *aes.CipherContext) ([]byte, error) {

	buf := new(bytes.Buffer)
	// 1. BodyLen: 4-byte unsigned integer
	bodyLen := uint32(48) // example value
	if err := ParseUnsignedInt(buf, uint64(bodyLen), 4); err != nil {
		return nil, fmt.Errorf("error parsing BodyLen: %w", err)
	}

	// 2. TemplateID: 2-byte unsigned integer
	templateID := uint16(10120)
	if err := ParseUnsignedInt(buf, uint64(templateID), 2); err != nil {
		return nil, fmt.Errorf("error parsing TemplateID: %w", err)
	}
	...
	// 7. SecurityID: 8-byte signed int
	SecurityID := int32(instrument_id)
	if err := ParseSignedInt(buf, int64(SecurityID), 8); err != nil {
		return nil, fmt.Errorf("error parsing MarketSegmentID: %w", err)
	}

	// 8. MarketSegmentID: 4-byte signed int
	marketSegmentID := int32(product_id)
	if err := ParseSignedInt(buf, int64(marketSegmentID), 4); err != nil {
		return nil, fmt.Errorf("error parsing MarketSegmentID: %w", err)
	}

	// 9. RegulatoryID: 4-byte unsigned integer
	regulatoryID := uint32(1234)
	if err := ParseUnsignedInt(buf, uint64(regulatoryID), 4); err != nil {
		return nil, fmt.Errorf("error parsing RegulatoryID: %w", err)
	}
