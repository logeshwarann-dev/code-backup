...
func (t Traders) handleOrderEntry() error {

	startTime := time.Now()
	totalTime := time.Now()
	packetCount := 0
	totalPacketCount := 0
	// batchSize := int(t.ThrottleValue)
	...
	for data := range t.TraderChan {
		...
		if utils.MARKET_REPLAY {
			...
			case utils.ORDER_ENTRY:
				...
				_, _, _, err := t.RetryOrderEntry(data.InstID, int(data.Price), 1)
				if err != nil {
...
func (t Traders) RetryOrderEntry(inst_id, price, retryCount int) (uint64, uint64, uint16, error) {
	...
	if order_template == utils.HEARTBEAT {
		t.RetryOrderEntry(inst_id, price, retryCount+1)
	}

	return order_id, act_time, order_template, nil
}

func (t Traders) SendHeartBeat() {
...
func checkConnectedTraders() {
	for {
		count := int(math.Floor(float64(connectedTraders*90) / 100))

		if count >= utils.TRADERCOUNT {
			sendOrders = true
			return
		}
	}
...
func (t Traders) handleOrderEntryTest() error {
	...
	for {
		...
		case <-time.After(1 * time.Second):
			...
			fmt.Println("[INFO] Sending heartbeat...")
			heartbeat, err := utils.HeartBeatRequest()
			if err != nil {
