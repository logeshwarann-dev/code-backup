...
func (t *Trader) createConnection() (net.Conn, error) {
	...
	fmt.Println("[INFO] Primary GW IP and Port: ", t.HostIP, t.Port)
...
func (t *Trader) NewOrderResponseListner(inst_id int) (uint64, uint64, uint16, error) {
	...
	for {
		...
		if len(responseBuf) > 0 {
			fmt.Printf("[INFO] Response received for Server %v: %v\n", t.session_id, string(responseBuf[:]))
			return 0, 0, 0, nil
		} else {
			fmt.Printf("Error response: %v", string(responseBuf[:]))
			return 0, 0, 0, nil
		}
		...
		dataChanLoop:
			for data := range recordsChan {
				...
				for loopCount < ORDER_ENTRY_COUNT {
					...
					orderRequest, err := SingleLegLeanOrder(t.MID, t.TID, t.msg_seq, record.InstrumentId, price, qty, order_type, t.ClOrdID, client_code)
					if err != nil {
						return err
					}
					...
					_, _, _, err = t.NewOrderResponseListner(record.InstrumentId)
					if err != nil {
						return fmt.Errorf("response error for session id %d: %v", t.session_id, err)
					}
