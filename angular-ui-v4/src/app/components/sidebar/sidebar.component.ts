...
export class ConfigPanelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.peakGeneratorForm = this.fb.group({
      ...
      process_type: [0, Validators.required],
      app_system_vendor: [''], 
      app_system_version: ['']
    });

    this.peakGeneratorForm.get('process_type')?.valueChanges.subscribe(value => {
      if (value === 1) {
        this.peakGeneratorForm.get('app_system_vendor')?.setValidators(Validators.required);
        this.peakGeneratorForm.get('app_system_version')?.setValidators(Validators.required);
      } else {
        this.peakGeneratorForm.get('app_system_vendor')?.clearValidators();
        this.peakGeneratorForm.get('app_system_version')?.clearValidators();
      }

      this.peakGeneratorForm.get('app_system_vendor')?.updateValueAndValidity();
      this.peakGeneratorForm.get('app_system_version')?.updateValueAndValidity();
    });
  ...
  updateThrottle() {
    ...
    if ( this.throttleValue == null || this.throttleValue < 0) {
      this.messageService.add({severity: 'warn', summary: 'Warning', detail: 'Please enter a valid Throttle Value.'});
  ...
  validateInput(field: keyof ConfigSettings): void {
    const validationMap: Record<string, (value: number) => string | undefined> = {
      numTraders: (value) => (value < 100 || value > 1300 ? 'Number of traders must be between 100 and 1300.' : value%100 !=0 ? "Number of traders must be multiple of 100": undefined)
    };
  ...
  getConfig(): void{
    this.apiService.fetchConfig().subscribe((response : any)=>{
      ...
      this.config = {
        numTraders: configData.TraderCount || 100,
        marketReplay: false, 
  ...
  prepareConfigData() {
    return {
      trader_count: this.config.numTraders ? Math.max(Math.floor(this.config.numTraders / 100) * 100, 100) : 100, // Required
  ...
  generatePeak() {
    ...
    let payload:any = {
      ...
      min: min_th,
      max: max_th,
      process_type: this.peakGeneratorForm.value.process_type,
      delay_min : this.peakGeneratorForm.value.min_delay, 
      delay_max : this.peakGeneratorForm.value.max_delay, 
    };

    if (this.peakGeneratorForm.value.process_type ==1){
      payload.app_system_vendor =  this.peakGeneratorForm.value.app_system_vendor
      payload.app_system_version = this.peakGeneratorForm.value.app_system_version
    }
